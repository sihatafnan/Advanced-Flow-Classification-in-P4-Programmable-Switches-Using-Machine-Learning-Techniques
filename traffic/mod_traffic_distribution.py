...
class Traffic_simulator():
	def __init__(self):
		...
		self.iat_threshold = [70000, 50000, 20000, 10000, 5000, 1000]
		self.no_size1 = 0
		self.no_size2 = 0
		self.no_size3 = 0
		self.no_size4 = 0
	...
	def insert_tuple(self, flow, sIp, dIp, sPort, dPort, proto):
		if sIp in flow.keys():
			if dIp in flow[sIp].keys():
				if proto in flow[sIp][dIp].keys():
					if sPort in flow[sIp][dIp][proto].keys():
						flow[sIp][dIp][proto][sPort][dPort] = {}
					else:
						flow[sIp][dIp][proto][sPort] = {}
						flow[sIp][dIp][proto][sPort][dPort] = {}
				else:
					flow[sIp][dIp][proto] = {}
					flow[sIp][dIp][proto][sPort] = {}
	...
	def exist_tuple(self, flow, sIp, dIp, sPort, dPort, proto):
		if sIp in flow.keys():
			if dIp in flow[sIp].keys():
				if proto in flow[sIp][dIp].keys():
					if sPort in flow[sIp][dIp][proto].keys():
						if dPort in flow[sIp][dIp][proto][sPort].keys():
							return True
						else:
							return False
					else:
	...
	def generate_flow(self, flows, hosts, ip_list, port_list, proto_list):
		...
		while flow_count < flows:
			# print(flow_count)
			first_ip_list = ip_list[:]
			second_ip_list = ip_list[:]
			# print(flow_count)second_ip_list.pop(receiver_ip_index)
			# print("===")
			while len(first_ip_list) != 0:
				...
				if sender_ip_index == receiver_ip_index and len(first_ip_list) != 1 and len(second_ip_list) != 1:
					continue
				...
				inserted1 = 0
				inserted2 = 0
				if not self.exist_tuple(flow, first_ip_list[sender_ip_index], second_ip_list[receiver_ip_index], port_list[sender_port_index], port_list[receiver_port_index], proto_list[proto_index]):
					
					flow1 = Flow(first_ip_list[sender_ip_index], second_ip_list[receiver_ip_index], port_list[sender_port_index], port_list[receiver_port_index], proto_list[proto_index])
					self.insert_tuple(flow, first_ip_list[sender_ip_index], second_ip_list[receiver_ip_index], port_list[sender_port_index], port_list[receiver_port_index], proto_list[proto_index])
					inserted1 = 1
					self.set_list(flow1, size_dist)
					flow_count = flow_count + 1
					first_ip_list.pop(sender_ip_index)
					second_ip_list.pop(receiver_ip_index)
					print(len(first_ip_list))
					print(len(second_ip_list))
					if flow_count >= flows:
						break

				# if not self.exist_tuple(flow, second_ip_list[receiver_ip_index], first_ip_list[sender_ip_index], port_list[receiver_port_index], port_list[sender_port_index], proto_list[proto_index]):

				# 	flow2 = Flow(second_ip_list[receiver_ip_index], first_ip_list[sender_ip_index], port_list[receiver_port_index], port_list[sender_port_index], proto_list[proto_index])
				# 	self.insert_tuple(flow, second_ip_list[receiver_ip_index], first_ip_list[sender_ip_index], port_list[receiver_port_index], port_list[sender_port_index], proto_list[proto_index])
				# 	inserted2 = 1
				...
				# if inserted1 == 1:
					# print(len(first_ip_list))
					# print(len(second_ip_list))
					# print(sender_ip_index)
					# print(receiver_ip_index)
	...
	def get_size_distr(self, distr_type):
		if distr_type == "basic":
			size_distr = [0.85, 0.07, 0.04, 0.025, 0.015]
		return size_distr
...
def main():
	...
	for i in range(hosts):
		...
		# 	addr1 = addr1 + 1
		# 	addr2 = 0
		ip_list.append("10.0.%d.%d" % (addr1,addr1))
		# addr2 = addr2 + 1
		addr1 = addr1 + 1
        print(ip_list)

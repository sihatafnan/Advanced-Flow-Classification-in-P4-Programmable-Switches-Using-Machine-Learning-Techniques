...
def send_pkt(flow, iface):
    ...
    siz = f_siz/f_cnt
    pkt =  Ether(src=iface_hwaddr, dst=flow[9])
    if int(flow[4]) == 6:
        pkt = pkt /IP(src=flow[0], dst=flow[1], proto=int(flow[4]), len=int(siz)) / TCP(dport=int(flow[3]), sport=int(flow[2])) / "batch-"+flow[10]
    elif int(flow[4]) == 17:
        pkt = pkt /IP(src=flow[0], dst=flow[1], proto=int(flow[4]), len=int(siz)) / UDP(dport=int(flow[3]), sport=int(flow[2])) / "batch-"+flow[10]
    pkt.show2()

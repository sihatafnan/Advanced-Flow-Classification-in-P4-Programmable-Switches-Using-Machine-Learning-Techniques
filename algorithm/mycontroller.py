...
def find_feature(textfile):
    ...
    for i in range(3):
        ...
        # print(line)
        if "count = [" in line:
            count = re.findall('\d+', line)
            count = [int(i) for i in count]
            print(count)
        # line = f.readline()
        # print(line)
        if "int_arr_time = [" in line:
            iat = re.findall('\d+', line)
            iat = [int(i) for i in iat]
            print(iat)
    f.close
    return iat, size, count
...
def find_classification(textfile, proto, src, dst):
    ...
    for line in f:
        ...
        if n:
            fea.append(re.findall(r"(int_arr_time|size|count)", line))
